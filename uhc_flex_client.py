...
class UhcFlexConfig:
    """
    Optum FLEX public Provider Directory endpoints.

    Defaults to UHC payer id "hsid" and the R4 base path.
    """
    ...
    scope: Optional[str] = None  # e.g. "public/Location.read"

    @staticmethod
    def from_env() -> "UhcFlexConfig":
        payer_id = os.getenv("UHC_FLEX_PAYER_ID", "hsid").strip() or "hsid"
        default_base = f"https://{payer_id}.fhir.flex.optum.com/R4/"
        return UhcFlexConfig(
            base_url=os.getenv("UHC_FLEX_BASE_URL", default_base),
            bearer_token=os.getenv("UHC_FLEX_BEARER_TOKEN") or None,
            token_url=os.getenv("UHC_FLEX_TOKEN_URL") or None,
            client_id=os.getenv("UHC_FLEX_CLIENT_ID") or None,
            client_secret=os.getenv("UHC_FLEX_CLIENT_SECRET") or None,
            scope=os.getenv("UHC_FLEX_SCOPE") or None,
        )

    @staticmethod
    def public_sandbox() -> "UhcFlexConfig":
        # Some environments may not resolve this host; keep as an explicit opt-in.
        return UhcFlexConfig(base_url="https://public.fhir.stage.flex-nonprod.optum.com/R4/")


class _ClientCredentialsOAuth:
    def __init__(self, cfg: UhcFlexConfig):
        self._cfg = cfg
    ...
    def _get_json(self, url: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        for attempt in range(1, retries + 1):
            ...
            if resp.status_code in (401, 403):
                # FLEX often exposes capability metadata publicly, while directory data may require
                # a client_credentials token with public/*.read scopes. Surface clearly.
                resp.raise_for_status()
